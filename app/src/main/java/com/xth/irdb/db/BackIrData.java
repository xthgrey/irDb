...
//            setAirContolData(id);
    ...
    private byte[] analyzeData(int electricType, byte[] irData, int key,int id) {
        byte checkSum = 0;
        byte[] tempArray = null;
        switch (electricType) {
            case Constants.air:
                tempArray = new byte[13 + irData.length];
                byte[] airContolDataArray = airData.airContolData(key, id);
                tempArray[0] = 0x30;
                tempArray[1] = 0x01;
    ...
    public byte[] modelOrIntelligentMatch(int matchType, int electricType, int serial, int key,int id) {
        if (mId != id) {
            mId = id;
            irData = dbManage.matchDb(matchType, electricType, serial);
            if (electricType == Constants.air) {
                airData = new AirData(id);
                getAirContolData(id);
            }
        }
        return analyzeData(electricType, irData, key,id);
    }
    protected byte[] oneKeyMatch(int electricType, byte[] learnData,int brandIndex) {
